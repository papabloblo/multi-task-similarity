...
similarity <- read_csv("empirical-work/bikes-autoencoder/data/similarity_tasks_matrix-spline.csv")

ale <- read_csv("empirical-work/bikes-autoencoder/data/ales-spline.csv")
...
p <- ale %>% 
  # filter(task %in% c(38, 76, )) %>% 
  mutate(cluster = case_when(
    task %in% c(38, 76, 129, 146) ~ 'Cluster 1',
    task %in% c(64, 97, 149, 250) ~ 'Cluster 3',
  )) %>%
...
ggsave(
  plot = p,
  filename = "empirical-work/bikes-autoencoder/plots/ales-splines-clusters.png", 
  width = 40,
  height = 20,
  dpi = "print",
  units = "cm"
