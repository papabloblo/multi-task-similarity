...
similarity <- read_csv("empirical-work/bikes/data/similarity_matrix.csv")
simi_task_features <- read_csv("empirical-work/bikes/data/similarity_matrix_task_feature.csv")
ale <- read_csv("empirical-work/bikes/data/ALE.csv")

similarity$...1 <- NULL

var_names <- c('var0' = 'Month', 
               'var1' = 'Weekday',
               'var2' = 'Hour', 
               'var3' = 'Wind speed',
               'var4' = 'Wind direction',
...
ale$feature2 <- var_names[ale$feature]

ale$task <- paste('Task', str_remove(ale$task, 'task'))
...
join_clusters <- function(ale, hclust, n_clusters){
  ...
  names_tasks <- paste('Task', names(cut_clusters))
...
plot_ale_by_cluster_feature <- function(ale_clus, 
  ...
  if (by_cluster){
    p <- p + facet_wrap(cluster ~ feature2, scales = "fixed", ncol = 1)
  } else {
    p <- p + facet_wrap( .~ feature2, scales = "fixed", ncol = 1)
  }
...
ale <- ale %>% 
  filter(
    x!=0,
    # !(x > 40 & feature2 == 'Wind speed'),
    # !(x > 150 & feature2 == 'Humidity'),
    ) %>% 
  group_by(task, feature) %>% 
  mutate(ale_centered = ale - mean(ale)) %>% 
  ungroup()

ale <- join_clusters(ale, hclust_avg, 6)
